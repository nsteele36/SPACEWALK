    ...
    public class Player : Sprite
    {
        ...
        public void Update(GameTime gameTime, List<Road> _road1, List<Road> _road2, List<Road> _road3, SoundEffect jumpSound, SoundEffect crouchSound, SoundEffect levelUpSound)
        {
            ...
            if (getWorld().ContactCount > 3)
            {
                ...
                crashed = true;
                crashedTime = (float)gameTime.ElapsedGameTime.TotalSeconds;
                getWorld().Gravity = new Vector2(0, 0);
            }
            ...
            if (isLevelUp && obstaclesPassed % 6 != 0)
                isLevelUp = false;
        ...
        public bool initializePuck()
        {
            try
            {
                _puck = new HIDPuckDongle(Game.Activity);
                _puck.Open();
            }
            catch (Exception e)
            {
